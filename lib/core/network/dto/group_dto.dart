...
///
/// Normalizes all formats to a simple `{ name, description }` structure.
library;
...
class GroupDto {
  ...
  const GroupDto({required this.name, this.description, this.id});
  ...
  static GroupDto fromCommonUserGroup(
    ...
    return GroupDto(name: groupName, description: json['desc'] as String?);
  }
  ...
  static GroupDto fromSub2ApiGroup(Map<String, dynamic> json) {
    return GroupDto(
      id: json['id']?.toString(),
  ...
  @override
  String toString() =>
      'GroupDto(name: $name, description: $description, id: $id)';
